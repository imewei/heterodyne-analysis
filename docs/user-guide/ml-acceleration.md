...
- Extracted **5 optimization records** from your recent heterodyne analysis results
...
heterodyne --ml-accelerated --config my_config.json

# Train models before analysis (recommended for better predictions)
heterodyne --ml-accelerated --train-ml-model --config my_config.json

# Use transfer learning from similar conditions
heterodyne --ml-accelerated --enable-transfer-learning --config my_config.json
```
...
heterodyne --ml-accelerated --ml-data-path ./my_ml_data --config my_config.json
```
...
heterodyne --distributed --ml-accelerated --config my_config.json

# ML acceleration with robust methods
heterodyne --ml-accelerated --method robust --laminar-flow --config my_config.json
```
...
1. **Run more analyses** with different conditions:
   ...
   ```bash
   heterodyne --ml-accelerated --config config1.json
   heterodyne --ml-accelerated --config config2.json
   heterodyne --ml-accelerated --config config3.json
   ```
...
1. Check heterodyne documentation
2. Review test cases in `heterodyne/tests/test_ml_acceleration.py`
3. Submit issues to heterodyne repository
