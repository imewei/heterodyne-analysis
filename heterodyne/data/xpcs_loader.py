...
class XPCSDataLoader:
    ...
    def load_experimental_data(self) -> tuple[np.ndarray, int, np.ndarray, int]:
        ...
        # Apply normalization if enabled in preprocessing config
        preprocessing = self.exp_config.get("preprocessing", {})
        if preprocessing.get("normalize_data", False):
            normalize_method = preprocessing.get("normalization_method", "baseline")
            logger.info(f"Applying {normalize_method} normalization to correlation data")
            c2_experimental = self._normalize_correlation_data(c2_experimental, normalize_method)
    ...
    def _normalize_correlation_data(self, c2_exp: np.ndarray, method: str = "baseline") -> np.ndarray:
        """
        Normalize correlation data to bring g₂ values to physical range around 1.

        Parameters
        ----------
        c2_exp : np.ndarray
            Correlation data with shape (n_angles, n_t2, n_t1)
        method : str
            Normalization method. Options:
            - "baseline": Divide by minimum value per angle (brings min to 1.0)
            - "minmax": Scale to [1.0, 2.0] range per angle
            - "global_baseline": Divide by global minimum across all angles
        ...
        """
        c2_normalized = c2_exp.copy()

        if method == "baseline":
            # Normalize each angle by its minimum value to bring baseline to 1.0
            for i in range(c2_normalized.shape[0]):
                min_val = np.min(c2_normalized[i])
                if min_val > 0:  # Avoid division by zero
                    c2_normalized[i] = c2_normalized[i] / min_val
                    logger.debug(f"Angle {i}: normalized by baseline={min_val:.3f}")
                else:
                    logger.warning(f"Angle {i}: skipping normalization (min_val={min_val:.3f} <= 0)")

        elif method == "minmax":
            # Scale each angle to [1.0, 2.0] range
            for i in range(c2_normalized.shape[0]):
                min_val = np.min(c2_normalized[i])
                max_val = np.max(c2_normalized[i])
                if max_val > min_val:
                    c2_normalized[i] = 1.0 + (c2_normalized[i] - min_val) / (max_val - min_val)
                    logger.debug(f"Angle {i}: scaled [{min_val:.3f}, {max_val:.3f}] → [1.0, 2.0]")
                else:
                    logger.warning(f"Angle {i}: skipping normalization (max={max_val:.3f} == min={min_val:.3f})")

        elif method == "global_baseline":
            # Normalize all angles by global minimum
            global_min = np.min(c2_normalized)
            if global_min > 0:
                c2_normalized = c2_normalized / global_min
                logger.debug(f"Global baseline normalization: divided by {global_min:.3f}")
            else:
                logger.warning(f"Skipping global normalization (global_min={global_min:.3f} <= 0)")

        else:
            logger.warning(f"Unknown normalization method '{method}', skipping normalization")

        # Log normalization results
        for i in range(min(3, c2_normalized.shape[0])):  # Log first 3 angles
            min_val = np.min(c2_normalized[i])
            max_val = np.max(c2_normalized[i])
            mean_val = np.mean(c2_normalized[i])
            logger.info(f"Angle {i} after normalization: min={min_val:.3f}, max={max_val:.3f}, mean={mean_val:.3f}")

        return c2_normalized
