...
Tests for the new heterodyne/core/optimization_utils.py module created during
...
def test_optimization_counter_functionality():
    ...
    from heterodyne.core.optimization_utils import get_optimization_counter
    from heterodyne.core.optimization_utils import increment_optimization_counter
    from heterodyne.core.optimization_utils import reset_optimization_counter
    ...
    for i in range(2, 6):  # Test counts 2-5
        ...
        assert (
            get_optimization_counter() == expected
...
def test_numba_availability_detection():
    ...
    if "heterodyne.core.optimization_utils" in sys.modules:
        importlib.reload(sys.modules["heterodyne.core.optimization_utils"])

    from heterodyne.core.optimization_utils import NUMBA_AVAILABLE
...
def test_integration_with_classical_optimizer():
    ...
    try:
        from heterodyne.core.optimization_utils import get_optimization_counter
        from heterodyne.optimization import ClassicalOptimizer
...
def test_integration_with_analysis_core():
    ...
    if "heterodyne.analysis.core" in sys.modules:
        importlib.reload(sys.modules["heterodyne.analysis.core"])

    try:
        # Test that we can import the increment function that analysis core uses
        from heterodyne.analysis.core import NUMBA_AVAILABLE
        from heterodyne.core.optimization_utils import increment_optimization_counter
...
def test_module_imports_correctly():
    ...
    try:
        ...
        import heterodyne.core.optimization_utils as opt_utils

        # Verify all expected attributes are present
        assert hasattr(opt_utils, "NUMBA_AVAILABLE"), "Module missing NUMBA_AVAILABLE"
        assert hasattr(
            opt_utils, "OPTIMIZATION_COUNTER"
        ), "Module missing OPTIMIZATION_COUNTER"
        assert hasattr(
            opt_utils, "get_optimization_counter"
        ), "Module missing get_optimization_counter"
        assert hasattr(
            opt_utils, "reset_optimization_counter"
        ), "Module missing reset_optimization_counter"
        assert hasattr(
            opt_utils, "increment_optimization_counter"
