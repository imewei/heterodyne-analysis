...
Enterprise-grade automated import detection and removal tooling for the heterodyne package.
...
class ImportInfo(NamedTuple):
    ...
    module: str
    name: str | None  # None for module imports, specific name for from imports
    alias: str | None
    line: int
    col_offset: int
    is_type_only: bool
...
class AdvancedImportAnalyzer(ast.NodeVisitor):
    ...
    def _analyze_text_content(self):
        ...
        for line_num, line in enumerate(self.source_lines, 1):
            ...
            if comment_match:
                ...
                self._extract_names_from_text(
                    comment_text, self.usage_context.comment_usage
                )
            ...
            for string_content in string_matches:
                self._extract_names_from_text(
                    string_content, self.usage_context.string_usage
                )
    ...
    def _is_conditional_import(self, node) -> bool:
        ...
        return any(
            isinstance(ctx, (ast.If, ast.Try, ast.ExceptHandler))
...
class EnterpriseImportAnalyzer:
    ...
    def _find_python_files(self) -> list[Path]:
        ...
        for path in self.package_root.rglob("*.py"):
            ...
            if any(
                part in exclude_patterns or part.startswith(".") for part in path.parts
            ):
    ...
    def analyze_file(self, file_path: Path, use_cache: bool = True) -> dict:
        ...
        analysis_result = {
            ...
            "from_imports": {
                f"{k[0]}::{k[1]}": v._asdict() for k, v in analyzer.from_imports.items()
            },
            "usage_context": {
                "runtime_usage": list(analyzer.usage_context.runtime_usage),
                "type_annotation_usage": list(
                    analyzer.usage_context.type_annotation_usage
                ),
            ...
            "attribute_access": {
                k: list(v) for k, v in analyzer.attribute_access.items()
            },
    ...
    def analyze_all_files(
        self, use_cache: bool = True, show_progress: bool = True
    ) -> dict[str, dict]:
    ...
    def find_unused_imports(
        ...
        for file_path, analysis in analysis_results.items():
            ...
            if "file_hash" in analysis:
                ...
                for import_key, info_dict in analysis["from_imports"].items():
                    ...
                    if name != "*" and not self._is_import_used(
                        info, analysis, file_content
                    ):
                        if self.safety_checker.check_import_safety(info, file_content):
                            unused_imports.append(
                                {
        ...
        self.logger.info(
            f"Found {total_unused} unused imports across {len(unused_by_file)} files"
        )
    ...
    def _is_import_used(
        self, info: ImportInfo, analysis: dict, file_content: str
    ) -> bool:
        ...
        if (
            alias in analysis["usage_context"]["string_usage"]
            or alias in analysis["usage_context"]["comment_usage"]
        ):
            return True
    ...
    def suggest_optimizations(self, analysis_results: dict[str, dict]) -> list[dict]:
        ...
        for module, files in module_usage.items():
            if len(files) > 3 and not module.startswith("heterodyne"):
                suggestions.append(
                    {
                        ...
                        suggestions.append(
                            {
                                ...
                                "impact_score": min(
                                    len(attributes) * 10, 50
                                ),  # Score based on number of attributes
                            }
                        )
        ...
        for file_path, unused_list in unused_imports.items():
            ...
            for unused in sorted_unused:
                ...
                if unused["type"] == "import":
                    ...
                    script_lines.append(
                        f"        # Remove: from {unused['module']} import {unused['name']}"
                    )
...
python {package_root}/heterodyne/tests/import_analyzer.py --check-only --package-root {package_root}
...
jobs:
  import-analysis:
    ...
    - name: Run import analysis
      run: |
        python heterodyne/tests/import_analyzer.py --check-only --external-tools
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enterprise-grade import analysis and automation for heterodyne package",
...
""",
    ...
    parser.add_argument(
        "--cleanup-script", type=Path, help="Generate cleanup script at this path"
    )
    parser.add_argument(
        "--check-only",
        action="store_true",
    ...
    # Enterprise features
    parser.add_argument(
        "--auto-cleanup", action="store_true", help="Generate automated cleanup script"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-cache", action="store_true", help="Disable analysis caching"
    )
    parser.add_argument(
        "--safety-level",
    ...
    parser.add_argument(
        "--generate-report", type=Path, help="Generate comprehensive analysis report"
    )
    parser.add_argument(
        "--config", type=Path, help="Configuration file for analysis settings"
    )
    ...
    analysis_results = analyzer.analyze_all_files(
        use_cache=not args.no_cache, show_progress=args.verbose
    )
    ...
    print(f"\n{'=' * 60}")
    print("ENTERPRISE IMPORT ANALYSIS SUMMARY")
    print(f"{'=' * 60}")
        ...
        for file_path, unused_list in unused_imports.items():
            ...
            if args.verbose:
                for unused in unused_list:
                    ...
                    conditional_flag = (
                        " [CONDITIONAL]" if unused.get("is_conditional") else ""
                    )
                    type_flag = " [TYPE-ONLY]" if unused.get("is_type_only") else ""

                    if unused["type"] == "import":
                        print(
                            f"    {safety_icon} Line {unused['line']}: import {unused['module']}{conditional_flag}{type_flag}"
                        )
                    else:
                        print(
                            f"    {safety_icon} Line {unused['line']}: from {unused['module']} import {unused['name']}{conditional_flag}{type_flag}"
                        )
    ...
    if args.auto_cleanup and unused_imports:
        if args.dry_run:
            ...
            for file_path, unused_list in unused_imports.items():
                safe_imports = [
                    u for u in unused_list if u.get("safety_level") == "high"
                ]
                if safe_imports:
            ...
            script_path = analyzer.generate_safe_cleanup_script(
                unused_imports, args.cleanup_script
            )
            print(f"✓ Cleanup script generated: {script_path}")
            print("Review the script carefully before executing!")
    ...
    if total_unused > 0:
        ...
        safe_removals = sum(
            1
            for file_imports in unused_imports.values()
            for imp in file_imports
            if imp.get("safety_level") == "high"
        )
        if safe_removals > 0:
        ...
        print("2. Use --auto-cleanup --dry-run to preview changes")
        print("3. Generate cleanup script with --auto-cleanup")
        print("4. Consider --setup-hooks for continuous monitoring")
