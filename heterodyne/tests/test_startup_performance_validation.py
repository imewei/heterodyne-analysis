...
import heterodyne
print(f"Import successful: {heterodyne.__version__}")
                """,
                ],
                check=False,
                capture_output=True,
...
print(f"VERSION:{heterodyne.__version__}")
            """,
            ],
            check=False,
            capture_output=True,
...
with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
    futures = [executor.submit(import_heterodyne) for _ in range(3)]
...
class TestPerformanceRegression:
    ...
    def test_performance_monitoring_accuracy(self):
        ...
        perf_data = heterodyne.monitor_startup_performance(iterations=3)
    ...
    def test_performance_trend_validation(self):
        ...
        measurements = []
        for i in range(5):
            perf_data = heterodyne.monitor_startup_performance(iterations=2)
...
print(f"WARNINGS_TIME:{end - start:.6f}")
    ...
    def test_import_from_different_directories(self):
        ...
        # Get current sys.path to ensure heterodyne is importable
        import json
        import tempfile
...
# Restore sys.path so heterodyne can be imported
...
class TestPerformanceTargetValidation:
    ...
    def test_comprehensive_performance_validation(self):
        ...
        perf_data = heterodyne.monitor_startup_performance(iterations=5)
