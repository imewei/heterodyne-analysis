...
Tests to ensure all imports in the heterodyne package are properly used and optimized.
...
class ImportVerificationSuite:
                    ...
                    if module.startswith("."):
                        ...
                        if package_name:
                            try:
                                ...
                                if target_parts:
                                    resolved_module = "heterodyne." + ".".join(
                                        target_parts
                                    )
                                    try:
                                        full_module = importlib.import_module(
                        ...
                        try:
                            ...
                            if (
                                ...
                                or module.startswith("heterodyne.")
                                or module
                                in [
                                    "core",
                                    "analysis",
    ...
    def _get_package_name(self, file_path: Path) -> str:
        ...
        try:
            ...
            # Prepend 'heterodyne' if not already there
            if parts:
                if parts[0] != "heterodyne":
                    parts = ("heterodyne", *parts)
                return ".".join(parts)
            return "heterodyne"
        except ValueError:
            return ""
    ...
    def analyze_import_dependencies(self) -> dict[str, set[str]]:
        ...
        for file_path in self.python_files:
            ...
            # Add regular imports
            for module_name in imports.keys():
                if module_name.startswith("heterodyne"):
                    file_deps.add(module_name)

            # Add from imports
            for module, _ in from_imports.keys():
                if module and (module.startswith(("heterodyne", "."))):
                    if module.startswith("."):
...
def package_root():
    ...
    # Go up from tests to heterodyne package root
...
class TestImportOptimization:
    ...
    def test_numba_import_overhead(self):
        ...
        # Clear heterodyne kernels module
        if "heterodyne.core.kernels" in sys.modules:
            del sys.modules["heterodyne.core.kernels"]
        ...
        # Clear modules again
        to_remove = [
            name for name in sys.modules if name.startswith("heterodyne.core.kernels")
        ]
        for name in to_remove:
            del sys.modules[name]
    ...
    def test_conditional_imports_work(self):
        ...
        conditional_modules = [
            "heterodyne.optimization.robust",  # Should work without CVXPY
            "heterodyne.visualization.plotting",  # Should work without matplotlib
            "heterodyne.core.kernels",  # Should work without numba
        ]
    ...
    def test_import_time_regression(self):
        ...
        baselines = {
            "heterodyne": 0.5,
            "heterodyne.core": 0.3,
            "heterodyne.analysis": 0.8,
            "heterodyne.optimization": 1.0,
        }
