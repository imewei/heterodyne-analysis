...
class Test14ParameterRegression:
    ...
    def test_identical_ref_sample_produces_valid_correlation(self, config_14_param_identical):
        ...
        params = np.array([
            100.0, -0.5, 10.0,  # ref
            100.0, -0.5, 10.0,  # sample (identical)
    ...
    def test_different_ref_sample_changes_correlation(self, config_14_param_identical):
        ...
        params_different = np.array([
            100.0, -0.5, 10.0,   # ref (unchanged)
            120.0, -0.6, 12.0,   # sample (DIFFERENT)
            0.1, 0.0, 0.01,
            0.5, 0.0, 50.0, 0.3,
            0.0
        ])
        ...
        assert max_diff > 1e-6, f"Expected different correlations, got max_diff={max_diff}"
    ...
    def test_validation_catches_invalid_sample_params(self, config_14_param_identical):
        ...
        invalid_params = np.array([
            100.0, -0.5, 10.0,   # ref (valid)
            -50.0, -0.5, 10.0,   # sample D0 negative (INVALID)
            0.1, 0.0, 0.01,
            0.5, 0.0, 50.0, 0.3,
            0.0
        ])
    ...
    def test_migration_produces_backward_compatible_params(self):
        ...
        assert params_14[0:3] == params_14[3:6], "Sample should equal reference initially"
...
class Test14ParameterNumericalStability:
    ...
    def test_extreme_diffusion_difference(self, core_instance):
        ...
        params = np.array([
            10.0, -0.3, 1.0,      # ref (slow diffusion)
            500.0, -0.9, 50.0,    # sample (fast diffusion)
            0.1, 0.0, 0.01,
            0.5, 0.0, 50.0, 0.3,
            0.0
        ])
        ...
        assert np.all(c2 >= -1e-10), "Correlation should be non-negative (within numerical precision)"
