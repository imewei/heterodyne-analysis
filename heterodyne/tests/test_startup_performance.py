...
from heterodyne.tests.conftest import PerformanceTimer
...
class StartupBenchmarkSuite:
    ...
    def clear_heterodyne_modules(self):
        """Clear all heterodyne modules from sys.modules."""
        to_remove = [name for name in sys.modules if name.startswith("heterodyne")]
        for name in to_remove:
            del sys.modules[name]

    def measure_cold_import(self, module_name: str) -> dict[str, float]:
        """Measure cold import performance with memory tracking."""
        self.clear_heterodyne_modules()
        gc.collect()
        ...
        with self.memory_profiler.monitor_memory():
            with PerformanceTimer(f"Cold import {module_name}") as timer:
                try:
                    ...
                    return {"import_time": None, "memory_delta_mb": 0, "error": str(e)}
    ...
    def measure_warm_import(
        self, module_name: str, iterations: int = 5
    ) -> dict[str, float]:
    ...
    def benchmark_progressive_loading(self) -> dict[str, dict[str, float]]:
        """Benchmark progressive loading of heterodyne components."""
        results = {}
        cumulative_time = 0
        cumulative_memory = 0
        ...
        loading_sequence = [
            ("base", "heterodyne"),
            ("config", "heterodyne.core.config"),
            ("kernels", "heterodyne.core.kernels"),
            ("analysis", "heterodyne.analysis.core"),
            ("classical_opt", "heterodyne.optimization.classical"),
            ("robust_opt", "heterodyne.optimization.robust"),
            ("plotting", "heterodyne.visualization.plotting"),
        ]
    ...
    def benchmark_lazy_loading_effectiveness(self) -> dict[str, float]:
        ...
        basic_result = self.measure_cold_import("heterodyne")
        ...
        with self.memory_profiler.monitor_memory():
            with PerformanceTimer("Import with lazy access") as timer:
                try:
                    ...
                    _ = heterodyne.HeterodyneAnalysisCore
                    _ = heterodyne.ClassicalOptimizer
                    _ = heterodyne.plot_c2_heatmaps
                except (ImportError, AttributeError):
                    pass

        results["lazy_access_time"] = timer.elapsed_time or 0
        results["lazy_access_memory_mb"] = self.memory_profiler.memory_delta.get(
            "rss_delta_mb", 0
        )
    ...
    def benchmark_conditional_imports(self) -> dict[str, dict[str, float]]:
        ...
        conditional_modules = {
            "numba_kernels": ("heterodyne.core.kernels", "numba"),
            "robust_optimization": ("heterodyne.optimization.robust", "cvxpy"),
            "plotting": ("heterodyne.visualization.plotting", "matplotlib"),
        }
        ...
        for test_name, (module_name, dependency) in conditional_modules.items():
            ...
            results[test_name] = {
                ...
                "without_dependency_memory": without_dep_result.get(
                    "memory_delta_mb", 0
                ),
            }
...
class TestStartupPerformance:
    ...
    def test_basic_import_performance(self, startup_benchmark):
        ...
        result = startup_benchmark.measure_cold_import("heterodyne")
    ...
    def test_warm_import_performance(self, startup_benchmark):
        ...
        result = startup_benchmark.measure_warm_import("heterodyne")
    ...
    def test_progressive_loading_performance(self, startup_benchmark):
        ...
        if failed_stages:
            pytest.fail(
                "Performance issues in progressive loading:\n"
                + "\n".join(failed_stages)
            )

        if total_time > max_total_time:
            pytest.fail(
                f"Total loading time too slow: {total_time:.2f}s (max: {max_total_time}s)"
            )
    ...
    def test_startup_regression_detection(self, startup_benchmark):
        ...
        baselines = {
            "heterodyne": {"time": 1.0, "memory": 30.0},
            "heterodyne.core.config": {"time": 0.3, "memory": 10.0},
            "heterodyne.analysis.core": {"time": 1.5, "memory": 25.0},
            "heterodyne.optimization.classical": {"time": 2.0, "memory": 40.0},
        }
...
class TestStartupOptimization:
    ...
    def test_numba_compilation_overhead(self, startup_benchmark):
        ...
        with_numba = startup_benchmark.measure_cold_import("heterodyne.core.kernels")

        startup_benchmark.clear_heterodyne_modules()
        with patch.dict("sys.modules", {"numba": None}):
            without_numba = startup_benchmark.measure_cold_import(
    ...
    def test_import_order_optimization(self, startup_benchmark):
        ...
        orders = [
            ...
            ["heterodyne", "heterodyne.core.config", "heterodyne.analysis.core"],
            ...
            ["heterodyne", "heterodyne.visualization.plotting"],
        ]
    ...
    def test_circular_import_prevention(self):
        ...
        try:
            ...
            for component in major_components:
                if hasattr(heterodyne, component):
                    _ = getattr(heterodyne, component)
            ...
            for module in new_modules:
                if module.startswith("heterodyne"):
    ...
    def test_startup_memory_leaks(self, startup_benchmark):
        ...
        for _ in range(5):
            startup_benchmark.clear_heterodyne_modules()
            startup_benchmark.measure_cold_import("heterodyne")
            gc.collect()
...
def run_startup_benchmark_suite() -> dict[str, dict]:
    ...
    results = {
        "basic_import": suite.measure_cold_import("heterodyne"),
