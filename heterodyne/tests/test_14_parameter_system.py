...
class Test14ParameterValidation:
    ...
    def test_14_parameter_count_validation(self):
        ...
        valid_params = np.array([
            100.0, -0.5, 10.0,     # D_ref params
            100.0, -0.5, 10.0,     # D_sample params
    ...
    def test_parameter_array_shapes(self):
        ...
        params = np.array([100.0, -0.5, 10.0, 100.0, -0.5, 10.0, 0.1, 0.0, 0.01,
...
class Test14ParameterDefaults:
    ...
    def test_parameter_names_metadata(self):
        ...
        param_metadata = {
            "D0_ref": {"unit": "nm²/s", "description": "Reference transport coefficient J₀_ref"},
            "alpha_ref": {"unit": "dimensionless", "description": "Reference transport coefficient time-scaling exponent"},
            "D_offset_ref": {"unit": "nm²/s", "description": "Reference baseline transport coefficient J_offset_ref"},
            "D0_sample": {"unit": "nm²/s", "description": "Sample transport coefficient J₀_sample"},
            "alpha_sample": {"unit": "dimensionless", "description": "Sample transport coefficient time-scaling exponent"},
            "D_offset_sample": {"unit": "nm²/s", "description": "Sample baseline transport coefficient J_offset_sample"},
...
class Test14ParameterIO:
    ...
    def test_load_14_parameters_from_json(self, tmp_path):
        ...
        param_data = {
            "initial_parameters": {
                "values": [100.0, -0.5, 10.0, 100.0, -0.5, 10.0, 0.1, 0.0, 0.01,
                          0.5, 0.0, 50.0, 0.3, 0.0],
                "parameter_names": [
                    "D0_ref", "alpha_ref", "D_offset_ref",
                    "D0_sample", "alpha_sample", "D_offset_sample",
...
class Test14ParameterOptimization:
    ...
    def test_default_14_parameter_fallback(self):
        ...
        default_14 = [100.0, -0.5, 10.0, 100.0, -0.5, 10.0, 0.1, 0.0, 0.01,
                     0.5, 0.0, 50.0, 0.3, 0.0]
...
class Test14ParameterBounds:
    ...
    def test_parameter_scaling(self):
        ...
        params = np.array([100.0, -0.5, 10.0, 100.0, -0.5, 10.0, 0.1, 0.0, 0.01,
                          0.5, 0.0, 50.0, 0.3, 0.0])
        bounds = np.array([
            [0, 1000], [-2, 2], [0, 100], [0, 1000], [-2, 2], [0, 100],
            [-10, 10], [-2, 2], [-1, 1], [0, 1], [-1, 1], [0, 200], [0, 1], [-360, 360]
        ])
...
class Test14ParameterBackwardCompatibility:
    ...
    def test_migration_guidance_for_legacy_params(self):
        ...
        assert new_param_count > old_7_param_count, \
            "Heterodyne model requires more parameters than 7-parameter legacy model"
        assert new_param_count > old_11_param_count, \
